// link: https://atcoder.jp/contests/abc302/tasks/abc302_b B - Find snuke
// time: 2024/6/28 19:31:26 https://github.com/funcdfs
   ...
   for (int i = 0; i < n; i++){
      for (int j = 0; j < m; j++) {
         for (int k = 0; k < 8; k++) {
            s = "";
            for (int l = 0; l < 5; l++) {
               if (0 <= i+dx[k]*l && i+dx[k]*l < n && 0 <= j+dy[k]*l && j+dy[k]*l < m) {
                  s += g[i+dx[k]*l][j+dy[k]*l];
               }
            }
            if (s == t) {
               for (int l = 0; l < 5; l++) {
