// link: https://codeforces.com/contest/1345/problem/B B. Card Constructions
// time: 2024/6/30 20:11:38 https://github.com/funcdfs
...
auto solve(int _case) -> void {
   ...
   vector<int> tb(2, 0);
   tb[1] = 2;
   for (int i = 2; tb.back() <= n; i++) {
      tb.push_back(0);
      tb[i] = tb[i-1] + 3*i - 1;
   }
   int ans = 0;
   while (true) {
      int l = 1, r = int(tb.size()) - 1;
      while (l < r) {
         int mid = (l + r + 1) >> 1;
         if (tb[mid] > n) {
            r = mid - 1;
