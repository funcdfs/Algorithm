// link: https://codeforces.com/contest/1200/problem/E E. Compress Words
// time: 2024/7/3 15:03:26 https://github.com/funcdfs
...
auto solve() -> void {
   ...
   for (int i = 1; i < n; i++) {
      StringHash now(s[i]);
      const int hsLen = int(hs.s.size());
      const int nowLen = int(s[i].size());
      int repeatLen = 0;
      for (int len = 1; nowLen >= len && hsLen - len >= 0; len += 1) {
         if (hs.substr(hsLen - len, len) == now.substr(0, len)) {
            repeatLen = len;
         }
      }
      hs.add(s[i].substr(repeatLen));
   }
