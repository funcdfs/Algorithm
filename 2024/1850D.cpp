// link: https://codeforces.com/contest/1850/problem/D D. Balanced Round
// time: 2024/9/22 13:12:58 https://github.com/funcdfs
...
auto solve(int _case) -> void {
   ...
   // erase some number, make diff <= k
   sort(a.begin(), a.end());

   // output the smallest number count to delete
   dbg(a);

   int maxContinueLen = 0;
   for (int i = 0, j = 0; i < n; i = j) {
      for (j = i+1; j < n && a[j]-a[j-1] <= k; j++) {
      }
      maxContinueLen = max(maxContinueLen, j-i);
   }

   println(n - maxContinueLen);
