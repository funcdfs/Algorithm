...
auto solve() -> void {
   ...
   while (n--) {
      int op, x; cin >> op >> x;
      if (op == 1) mt.insert(x);
      if (op == 2) mt.erase(mt.upper_bound(x));
      if (op == 3) cout << mt.order_of_key(x)+1 << endl; 
      if (op == 4) cout << *mt.find_by_order(x-1) << endl; 
      if (op == 5) cout << *--mt.upper_bound(x) << endl; 
      if (op == 6) cout << *mt.lower_bound(x) << endl; 
   }
