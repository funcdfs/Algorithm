// link: https://codeforces.com/contest/1791/problem/C -> C. Prepend and Append
// time: 2025/3/11 16:16:20 https://github.com/funcdfs
...
auto solve(int _case) -> void {
   ...
   // left add 1, right add 0
   // left add 0, right add 1

   auto op = [](string& s) -> int { // usage: [=, &val1, &val2]
      int cnt = 0;
      for (int i = 0; i < ssize(s)/2; i++) {
         if (s[i]=='0' && s.rbegin()[i]=='1') {
            cnt++;
         } else if (s[i]=='1' && s.rbegin()[i]=='0') {
            cnt++;
         } else {
            break;
         }
      }
      return ssize(s)-2*cnt;
   };
