// link: https://codeforces.com/contest/1216/problem/B -> B. Shooting
// time: 2025/2/13 13:23:06 https://github.com/funcdfs
...
void solve() {
   ...
   cin >> n;
   vector<int> a(n, 0);
   vector<pair<int, int>> sa(n, {0, 0});
   cin >> a;
   for (int i = 0; i < n; i++) {
      sa[i] = {a[i], i};
   }

   sort(sa.begin(), sa.end(), [](const pair<int, int> &a, const pair<int, int> &b) {
