// link: https://codeforces.com/contest/1690/problem/B -> B. Array Decrements
// time: 2025/2/2 10:22:27 https://github.com/funcdfs
...
auto solve(int _case) -> void {
   ...
   auto check = [&]() -> bool {
      // minus all elements in a by 1, is one operation
      // make a == b
      int maxValue = 0;
      for (int i = 0; i < n; i++) {
         Max(maxValue, a[i]-b[i]);
         if (a[i] < b[i]) {
            return false;
         }
      }
      int maxIdx = ranges::max_element(b) - b.begin();
      if (b[maxIdx] == 0) {
         return true;
      } else {
         int diff = a[maxIdx] - b[maxIdx];
         int ok = 1;
         for (int i = 0; i < n; i++) {
