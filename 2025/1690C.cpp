// link: https://codeforces.com/contest/1690/problem/C -> C. Restoring the Duration of Tasks
// time: 2025/2/2 18:14:01 https://github.com/funcdfs
...
auto solve(int _case) -> void {
   ...
   int n = 0;
   cin >> n;
   vector<int> s(n, 0), f(n, 0);
   cin >> s >> f;

   // s represent the start time of the task, f represent the finish time of the task
   vector<int> duration(n, 0);
   for  (int i = 0; i <n; i++) {
      if (i == 0 ) {
         duration[i] = f[i]-s[i];
      } else {
         duration[i] = min(f[i]-s[i], f[i]-f[i-1]);
      }
   }
