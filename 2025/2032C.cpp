// link: https://codeforces.com/contest/2032/problem/C -> C. Trinity
// time: 2025/1/23 02:04:30 https://github.com/funcdfs
...
auto solve(int _case) -> void {
   ...
   // output one single number, represent the minimum operation count
   // for each x, y, z, there is a triangle can be construct
   // operation: make a[i] == a[j]

   sort(a.begin(), a.end());
   int ans = n - 1;
   for (int i = 0; i + 1 < n; i++) {
      int x = -distance(lower_bound(a.begin(), a.end(), a[i]+a[i+1]), a.begin());
      ans = min(ans, n-(x-i));
   }
   println("{}", ans);
