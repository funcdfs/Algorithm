// link: https://codeforces.com/contest/1675/problem/C -> C. Detective Task
// time: 2025/2/12 16:00:05 https://github.com/funcdfs
...
auto solve(int _case) -> void {
   ...
   cin >> s;
   int n = ssize(s);
   // this string is no possibility to have "01" substring, bacause the theft count is == 1
   // the string  is same like 1111xxxxx0000, the center part is the answer

   int zero = 0;
   while (zero < n && s[zero] != '0') {
      zero += 1;
   }
   // zero store the index of the first '0'
   int one = n-1;
   while (one >= 0 && s[one] != '1') {
      one -= 1;
   }
   // one store the index of the last '1'
   int ans = 0;
   for (int i = 0; i < n; i++) {
      if (i <= zero && one <= i) {
         ans += 1;
