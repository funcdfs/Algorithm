// link: https://codeforces.com/contest/1046/problem/C -> C. Space Formula
// time: 2025/1/23 02:31:30 https://github.com/funcdfs
...
void solve() {
   ...
   int value = s[d-1]+p[0];
   int miniIdx = n-1;
   // only the before content can make rank better
   for (int i = 0; i <= d-2; i++) {
      if (s[i]+p[miniIdx] <= value) {
         miniIdx -= 1;
         // when a p is used, the idx which name miniAdd need to move back
         nowRank -= 1;
         // when this case, nowRank get better
      }
   }
