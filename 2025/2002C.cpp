// link: https://codeforces.com/contest/2002/problem/C -> C. Black Circles
// time: 2025/1/26 17:40:52 https://github.com/funcdfs
...
auto solve(int _case) -> void {
   ...
   auto check = [&]() -> bool {
      // every single second, the len increase by 1
      auto dist = [&](circle a, circle b) -> double {
         return sqrt((a.x - b.x) * (a.x - b.x) + (a.y - b.y) * (a.y - b.y));
      };
      int strightDistance = dist(Start, End);
      for (int i = 0; i < n; i++) {
         if (dist(C[i], End) <= strightDistance) {
            return false;
         }
      }
      return true;
