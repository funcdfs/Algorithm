// link: https://codeforces.com/contest/2032/problem/A -> A. Circuit
// time: 2025/1/22 18:44:49 https://github.com/funcdfs
...
auto solve(int _case) -> void {
   ...
   // output the max number of light can be lighten
   // ^ == true. others == false

   int oneCnt = count_if(a.begin(), a.end(), [=](const auto &xx) { return xx == 1; });
   int zeroCnt = count_if(a.begin(), a.end(), [=](const auto &xx) { return xx == 0; });
   int maxLight = min(oneCnt, zeroCnt);
   int minLight = min(maxLight, ((oneCnt % 2) + (zeroCnt % 2))/2);
   println("{} {}", minLight, maxLight);
